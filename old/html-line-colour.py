...
if (len_f+1) % 4 != 0:
    sys.exit("ERROR: no. of lines in file not equal to ((multiple of 4) - 1). Please make sure your file is correctly formatted and has no trailing lines.")

f = open(passed_file)
for num in range((len_f+1) / 4):
    master = f.readline().rstrip('\r\n')
    slave = f.readline().rstrip('\r\n')
    colour_order = f.readline().rstrip('\r\n')
    f.readline() # forget about spacing line
